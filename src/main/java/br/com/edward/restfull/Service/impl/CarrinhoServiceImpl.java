...
public class CarrinhoServiceImpl implements CarrinhoService {
    ...
    public Carrinho adicionar(Integer qtd, Long idProduto) {
        ...
        if (Objects.nonNull(produto)) {
            ...
            Carrinho carrinho = this.getCarrinhos();
            carrinho.addItem(itemCarrinhoService.getItem(qtd, produto, carrinho));
    ...
    @Override
    public Carrinho finaliza(Long idCliente) {
    	Optional<Cliente> cliente = clienteService.findById(idCliente);
    	if(cliente.isPresent()) {	
    		return this.carrinhoRepository.save(this.getCarrinhos().fechaCarrinho(cliente.get())); 	
    	}
        throw new RuntimeErrorException(null, "Erro, cliente inexistente");
    } 
    ...
    private Carrinho getCarrinhos() {

        List<Carrinho> lista = this.carrinhoRepository.findAll();
        if (lista.isEmpty()) {
